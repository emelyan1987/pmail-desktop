...
#
# An instance of this class is always available as the `PlanckEnv.packages` global.
#
...
class PackageManager
  ...
  isPackageDisabled: (name) ->
    _.include(PlanckEnv.config.get('core.disabledPackages') ? [], name)
  ...
  installPackageFromPath: (packageSourceDir, callback) ->
    ...
    packagesDir = path.join(PlanckEnv.getConfigDirPath(), 'packages')
  ...
  observeDisabledPackages: ->
    @disabledPackagesSubscription ?= PlanckEnv.config.onDidChange 'core.disabledPackages', ({newValue, oldValue}) =>
