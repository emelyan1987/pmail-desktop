...
class ProposedTimeCalendarStore extends NylasStore {
  ...
  activate() {
    ...
    this.unsubscribers = [
      this.listenTo(SchedulerActions.changeDuration, this._onChangeDuration),
      this.listenTo(SchedulerActions.changeDuration, this._onChangeDuration),
      this.listenTo(SchedulerActions.clearProposals, this._onClearProposals),
      this.listenTo(SchedulerActions.addToProposedTimeBlock, this._onAddToBlock),
      this.listenTo(SchedulerActions.startProposedTimeBlock, this._onStartBlock),
      this.listenTo(SchedulerActions.endProposedTimeBlock, this._onEndBlock),
      this.listenTo(SchedulerActions.removeProposedTime, this._onRemoveProposedTime),
    ]
  }
  ...
  _convertBufferToProposedTimes() {
    ...
    console.log("ProposedTimeCalendarStore->_convertBufferToProposedTimes", minMoment, maxMoment);
  ...
  _onStartBlock = (newT) => {
    console.log("ProposedTImeCalendarStore->_onStartBlock", newT, newT.floor(30, 'minutes').unix());
    this._resetDragBuffer();
    this._dragBuffer.anchor = newT.floor(30, 'minutes').unix()
  }

  _onAddToBlock = (newT) => {
    console.log("ProposedTImeCalendarStore->_onAddToBlock", newT, newT.ceil(30, 'minutes').unix());
