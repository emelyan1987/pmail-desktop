...
export default class MetadataComposerToggleButton extends React.Component {
  ...
  _isDefaultOn() {
    return PlanckEnv.config.get(this._configKey())
  }
    ...
    return DraftStore.sessionForClientId(this.props.draftClientId).then((session)=> {
      ...
      .catch((error) => {
        ...
        if (this.props.stickyToggle) {
          PlanckEnv.config.set(this._configKey(), false)
        }

        let title = "Error"
        if (!(error instanceof APIError)) {
          PlanckEnv.reportError(error);
        ...
        PlanckEnv.showErrorDialog({title, message: this.props.errorMessage(error)});
      })
    }).finally(() => {
  ...
  _onClick = () => {
    ...
    if (this.props.stickyToggle) {
      PlanckEnv.config.set(this._configKey(), !this.state.enabled)
    }
