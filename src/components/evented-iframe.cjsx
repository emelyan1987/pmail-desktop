...
class EventedIFrame extends React.Component
  ...
  _onIFrameClick: (e) =>
    ...
    if target
      ...
      e.preventDefault()
      PlanckEnv.windowEventHandler.openLink(target: target, metaKey: e.metaKey)
  ...
  _onIFrameContextualMenu: (event) =>
    ...
    if linkTarget
      ...
      menu.append(new MenuItem({ label: "Open Link", click:( -> PlanckEnv.windowEventHandler.openLink({href}) )}))
    ...
    if imageTarget
      ...
      menu.append(new MenuItem({
        label: "Save Image...",
        click: ->
          PlanckEnv.showSaveDialog {defaultPath: srcFilename}, (path) ->
            return unless path
    ...
    if text.length > 0
      ...
      if process.platform is 'darwin'
        menu.append(new MenuItem({ label: "Look Up '#{textPreview}'", click:( -> PlanckEnv.getCurrentWindow().showDefinitionForSelection() )}))
