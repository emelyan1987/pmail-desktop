...
class Package
  ...
  @isBundledPackagePath: (packagePath) ->
    if PlanckEnv.packages.devMode
      return false unless PlanckEnv.packages.resourcePath.startsWith("#{process.resourcesPath}#{path.sep}")

    @resourcePathWithTrailingSlash ?= "#{PlanckEnv.packages.resourcePath}#{path.sep}"
  ...
  constructor: (@path, @metadata) ->
    ...
    if @metadata.appId
      if _.isString @metadata.appId
        ...
        @pluginAppId = @metadata.appId[PlanckEnv.config.get('env')] ? null
      else
        @pluginAppId = null
    else
      @pluginAppId = null
  ...
  enable: ->
    PlanckEnv.config.removeAtKeyPath('core.disabledPackages', @name)

  disable: ->
    PlanckEnv.config.pushAtKeyPath('core.disabledPackages', @name)
  ...
  activateNow: ->
    try
      ...
      if @requireMainModule()
        localState = PlanckEnv.packages.getPackageState(@name) ? {}
  ...
  activateConfig: ->
    ...
    if @mainModule?
      if @mainModule.config? and typeof @mainModule.config is 'object'
        PlanckEnv.config.setSchema @name, {type: 'object', properties: @mainModule.config}
      else if @mainModule.configDefaults? and typeof @mainModule.configDefaults is 'object'
        PlanckEnv.config.setDefaults(@name, @mainModule.configDefaults)
  ...
  activateStylesheets: ->
    ...
    for [sourcePath, source] in @stylesheets
      ...
      @stylesheetDisposables.add(PlanckEnv.styles.addStyleSheet(source, {sourcePath, priority, context}))
    @stylesheetsActivated = true

  activateResources: ->
    @activationDisposables = new CompositeDisposable
    @activationDisposables.add(PlanckEnv.keymaps.add(keymapPath, map)) for [keymapPath, map] in @keymaps
    @activationDisposables.add(PlanckEnv.menu.add(map['menu'])) for [menuPath, map] in @menus when map['menu']?
  ...
  activateServices: ->
    for name, {versions} of @metadata.providedServices
      for version, methodName of versions
        @activationDisposables.add PlanckEnv.packages.serviceHub.provide(name, version, @mainModule[methodName]())

    for name, {versions} of @metadata.consumedServices
      for version, methodName of versions
        @activationDisposables.add PlanckEnv.packages.serviceHub.consume(name, version, @mainModule[methodName].bind(@mainModule))

  loadKeymaps: ->
    if @bundledPackage and packagesCache[@name]?
      @keymaps = (["#{PlanckEnv.packages.resourcePath}#{path.sep}#{keymapPath}", keymapObject] for keymapPath, keymapObject of packagesCache[@name].keymaps)
    else
      @keymaps = @getKeymapPaths().map (keymapPath) -> [keymapPath, PlanckEnv.keymaps.readKeymap(keymapPath) ? {}]

  loadMenus: ->
    if @bundledPackage and packagesCache[@name]?
      @menus = (["#{PlanckEnv.packages.resourcePath}#{path.sep}#{menuPath}", menuObject] for menuPath, menuObject of packagesCache[@name].menus)
    else
  ...
  loadStylesheets: ->
    @stylesheets = @getStylesheetPaths().map (stylesheetPath) ->
      [stylesheetPath, PlanckEnv.themes.loadStylesheet(stylesheetPath, true)]
  ...
  loadGrammarsSync: ->
    ...
    for grammarPath in grammarPaths
      try
        grammar = PlanckEnv.grammars.readGrammarSync(grammarPath)
  ...
  loadGrammars: ->
    ...
    loadGrammar = (grammarPath, callback) =>
      PlanckEnv.grammars.readGrammar grammarPath, (error, grammar) =>
        if error?
          console.warn("Failed to load grammar: #{grammarPath}", error.stack ? error)
        else
          grammar.packageName = @name
  ...
  getMainModulePath: ->
    ...
    if @bundledPackage and packagesCache[@name]?
      if packagesCache[@name].main
        @mainModulePath = "#{PlanckEnv.packages.resourcePath}#{path.sep}#{packagesCache[@name].main}"
    ...
    for selector, commands of @getActivationCommands()
      for command in commands
        do (selector, command) =>
          ...
          @activationCommandSubscriptions.add PlanckEnv.commands.add selector, command, ->
          @activationCommandSubscriptions.add PlanckEnv.commands.onWillDispatch (event) =>
  ...
  isCompatible: ->
    ...
    if @path.indexOf(path.join(PlanckEnv.packages.resourcePath, 'node_modules') + path.sep) is 0
