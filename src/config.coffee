...
#
# An instance of this class is always available as the `PlanckEnv.config` global.
#
# ## Getting and setting config settings.
#
# ```coffee
# # Note that with no value set, ::get returns the setting's default value.
# PlanckEnv.config.get('my-package.myKey') # -> 'defaultValue'
#
# PlanckEnv.config.set('my-package.myKey', 'value')
# PlanckEnv.config.get('my-package.myKey') # -> 'value'
# ```
#
...
# ```coffee
# PlanckEnv.config.set('my-package.myKey', 'value')
# PlanckEnv.config.observe 'my-package.myKey', (newValue) ->
...
#
# ```coffee
# PlanckEnv.config.onDidChange 'my-package.myKey', ({newValue, oldValue}) ->
...
# PlanckEnv.config.get('my-package.anInt') # -> 12
#
# # The string will be coerced to the integer 123
# PlanckEnv.config.set('my-package.anInt', '123')
# PlanckEnv.config.get('my-package.anInt') # -> 123
#
# # The string will be coerced to an integer, but it must be greater than 0, so is set to 1
# PlanckEnv.config.set('my-package.anInt', '-20')
# PlanckEnv.config.get('my-package.anInt') # -> 1
# ```
#
...
#
# ```coffee
# PlanckEnv.config.set('my-package.thingVolume', '10')
# PlanckEnv.config.get('my-package.thingVolume') # -> 10
#
# # It respects the min / max
# PlanckEnv.config.set('my-package.thingVolume', '400')
# PlanckEnv.config.get('my-package.thingVolume') # -> 11
#
# # If it cannot be coerced, the value will not be set
# PlanckEnv.config.set('my-package.thingVolume', 'cats')
# PlanckEnv.config.get('my-package.thingVolume') # -> 11
# ```
#
# ### Supported Types
...
#
# # Then
# PlanckEnv.config.set('my-package.someSetting', 'true')
# PlanckEnv.config.get('my-package.someSetting') # -> true
#
# PlanckEnv.config.set('my-package.someSetting', '12')
# PlanckEnv.config.get('my-package.someSetting') # -> 12
# ```
#
# #### string
#
...
#
# ```coffee
# PlanckEnv.config.set('my-package.someSetting', '2')
# PlanckEnv.config.get('my-package.someSetting') # -> 2
#
# # will not set values outside of the enum values
# PlanckEnv.config.set('my-package.someSetting', '3')
# PlanckEnv.config.get('my-package.someSetting') # -> 2
#
# # If it cannot be coerced, the value will not be set
# PlanckEnv.config.set('my-package.someSetting', '4')
# PlanckEnv.config.get('my-package.someSetting') # -> 4
# ```
#
...
class Config
  ...
  # Created during initialization, available as `PlanckEnv.config`
  ...
  #
  # ```coffee
  # PlanckEnv.config.observe 'core.themes', (value) ->
  #   # do stuff with value
  # ```
  #
  ...
  #
  # ```coffee
  # PlanckEnv.config.get('core.themes')
  # ```
  #
  ...
  #
  # ```coffee
  # PlanckEnv.config.get('editor.tabLength', scope: ['source.ruby']) # => 2
  # ```
  #
  # This setting in ruby files might be different than the global tabLength setting
  #
  # ```coffee
  # PlanckEnv.config.get('editor.tabLength') # => 4
  # PlanckEnv.config.get('editor.tabLength', scope: ['source.ruby']) # => 2
  # ```
  #
  ...
  #
  # ```coffee
  # PlanckEnv.config.get('editor.tabLength', scope: @editor.getRootScopeDescriptor()) # => 2
  # ```
  #
  ...
  # PlanckEnv.config.get('editor.tabLength', scope: scopeDescriptor) # => 2
  # ```
  #
  ...
  #     {editor.getLastCursor().getScopeDescriptor()}
  #
  # Returns the value from PlanckMail's default settings, the user's configuration
  ...
  #
  # ```coffee
  # PlanckEnv.config.set('core.themes', ['ui-light', 'my-custom-theme'])
  # ```
  #
  ...
  # ```coffee
  # PlanckEnv.config.get('editor.tabLength') # => 4
  # PlanckEnv.config.get('editor.tabLength', scope: ['source.ruby']) # => 4
  # PlanckEnv.config.get('editor.tabLength', scope: ['source.js']) # => 4
  #
  # # Set ruby to 2
  # PlanckEnv.config.set('editor.tabLength', 2, scopeSelector: 'source.ruby') # => true
  #
  ...
  getDefault: ->
    Grim.deprecate("Use `::get(keyPath, {scope, excludeSources: [PlanckEnv.config.getUserConfigPath()]})` instead")
    if arguments.length is 1
      [keyPath] = arguments
    else
  ...
  # otherwise.
  isDefault: ->
    Grim.deprecate("Use `not ::get(keyPath, {scope, sources: [PlanckEnv.config.getUserConfigPath()]})?` instead")
    if arguments.length is 1
      [keyPath] = arguments
    else
      [scopeSelector, keyPath] = arguments
  ...
  initializeConfigDirectory: (done) ->
    ...
    templateConfigDirPath = fs.resolve(@resourcePath, 'dot-planck')
