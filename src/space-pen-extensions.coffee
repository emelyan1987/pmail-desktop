...
jQuery.fn.trigger = (eventName, data) ->
  if NativeEventNames.has(eventName) or typeof eventName is 'object'
    ...
    for element in this
      PlanckEnv.commands.dispatch(element, eventName, data)
    this
...
AddEventListener = (element, type, listener) ->
  if NativeEventNames.has(type)
    ...
    disposable = PlanckEnv.commands.add(element, type, listener)
...
jQuery.event.add = (elem, types, originalHandler, data, selector) ->
  ...
  JQueryEventAdd.call(this, elem, types, handler, data, selector, AddEventListener if PlanckEnv?.commands?)
...
jQuery.event.remove = (elem, types, originalHandler, selector, mappedTypes) ->
  ...
  JQueryEventRemove(elem, types, handler, selector, mappedTypes, RemoveEventListener if PlanckEnv?.commands?)
