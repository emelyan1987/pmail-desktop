...
class WindowEventHandler
  ...
  constructor: ->
    ...
    @subscribe ipcRenderer, 'open-path', (event, pathToOpen) ->
      unless PlanckEnv.project?.getPaths().length
        if fs.existsSync(pathToOpen) or fs.existsSync(path.dirname(pathToOpen))
          PlanckEnv.project?.setPaths([pathToOpen])

      unless fs.isDirectorySync(pathToOpen)
        PlanckEnv.workspace?.open(pathToOpen, {})

    @subscribe ipcRenderer, 'update-available', (event, detail) ->
      PlanckEnv.updateAvailable(detail)
    ...
    @subscribe ipcRenderer, 'command', (event, command, args...) ->
      ...
      PlanckEnv.commands.dispatch(activeElement, command, args[0])
    ...
    @subscribe $(window), 'unload', =>
      ...
      PlanckEnv.saveStateAndUnloadWindow()
      PlanckEnv.windowEventHandler?.unsubscribe()

    @subscribeToCommand $(window), 'window:toggle-full-screen', ->
      PlanckEnv.toggleFullScreen()
    ...
    @subscribeToCommand $(window), 'window:toggle-dev-tools', ->
      PlanckEnv.toggleDevTools()

    @subscribeToCommand $(window), 'window:open-errorlogger-logs', ->
      PlanckEnv.errorLogger.openLogs()
  ...
  handleNativeKeybindings: ->
    menu = null
    webContents = PlanckEnv.getCurrentWindow().webContents
  ...
  onKeydown: (event) ->
    PlanckEnv.keymaps.handleKeyboardEvent(event)
  ...
  showDevModeMessages: ->
    return unless PlanckEnv.isMainWindow()
