...
#
# An instance of this class is always available as the `PlanckEnv.themes` global.
module.exports =
class ThemeManager
  ...
  onDidAddStylesheet: (callback) ->
    Grim.deprecate("Use PlanckEnv.styles.onDidAddStyleElement instead")
  ...
  onDidRemoveStylesheet: (callback) ->
    Grim.deprecate("Use PlanckEnv.styles.onDidRemoveStyleElement instead")
  ...
  onDidUpdateStylesheet: (callback) ->
    Grim.deprecate("Use PlanckEnv.styles.onDidUpdateStyleElement instead")
  ...
  onDidChangeStylesheets: (callback) ->
    Grim.deprecate("Use PlanckEnv.styles.onDidAdd/RemoveStyleElement instead")
  ...
  getEnabledThemeNames: ->
    themeNames = PlanckEnv.config.get('core.themes') ? []
    themeNames = [themeNames] unless _.isArray(themeNames)
    themeNames = themeNames.filter (themeName) ->
      if themeName and typeof themeName is 'string'
        return true if PlanckEnv.packages.resolvePackagePath(themeName)
    ...
    if PlanckEnv.getWindowType() is 'onboarding'
      themeNames = []
  ...
  setEnabledThemes: (enabledThemeNames) ->
    Grim.deprecate("Use `PlanckEnv.config.set('core.themes', arrayOfThemeNames)` instead")
    PlanckEnv.config.set('core.themes', enabledThemeNames)
  ...
  setActiveTheme: (theme) ->
    base = @baseThemeName()
    PlanckEnv.config.set('core.themes', _.uniq [base, theme])
  ...
  getUserStylesheetPath: ->
    Grim.deprecate("Call PlanckEnv.styles.getUserStyleSheetPath() instead")
    PlanckEnv.styles.getUserStyleSheetPath()
  ...
  loadUserStylesheet: ->
    ...
    userStylesheetPath = PlanckEnv.styles.getUserStyleSheetPath()
    ...
    @userStyleSheetDisposable = PlanckEnv.styles.addStyleSheet(userStylesheetContents, sourcePath: userStylesheetPath, priority: 2)
  ...
  applyStylesheet: (path, text) ->
    @styleSheetDisposablesBySourcePath[path] = PlanckEnv.styles.addStyleSheet(text, sourcePath: path)
  ...
  activateThemes: ->
    ...
    # PlanckEnv.config.observe runs the callback once, then on subsequent changes.
    PlanckEnv.config.observe 'core.themes', =>
      @deactivateThemes()
