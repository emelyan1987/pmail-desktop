...
class SheetContainer extends React.Component
  ...
  render: =>
    ...
    <Flexbox direction="column" className="layout-mode-#{@state.mode}" style={overflow: 'hidden'}>
      ...
      {@_menubarContainerElement()}
    </Flexbox>

  _menubarContainerElement: =>
    {mainWindow} = PlanckEnv.getLoadSettings()
    return [] unless mainWindow

    <div name="Menubar" style={order:4, zIndex: 5} className="sheet-menubar">
      <Menubar />
    </div>

  _toolbarContainerElement: =>
    {toolbar} = PlanckEnv.getLoadSettings()
