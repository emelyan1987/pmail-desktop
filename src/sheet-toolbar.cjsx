...
class WindowTitle extends React.Component
  ...
  constructor: (@props) ->
    @state = PlanckEnv.getLoadSettings()

  componentDidMount: ->
    @unlisten = PlanckEnv.onWindowPropsReceived (windowProps) =>
      @setState PlanckEnv.getLoadSettings()
...
class ToolbarBack extends React.Component
  ...
  render: =>
    curSheet = WorkspaceStore.topSheet()
    console.log curSheet
    if curSheet.id == "CalendarSheet" || curSheet.id == "File" || curSheet.id == "Contact"
      return <div></div>
...
class ToolbarWindowControls extends React.Component
  ...
  render: =>
    <div name="ToolbarWindowControls" className="toolbar-window-controls alt-#{@state.alt}">
      <button tabIndex={-1} className="close" onClick={ -> PlanckEnv.close()}></button>
      <button tabIndex={-1} className="minimize" onClick={ -> PlanckEnv.minimize()}></button>
  ...
  _onMaximize: (event) =>
    if process.platform is 'darwin' and not event.altKey
      PlanckEnv.setFullScreen(!PlanckEnv.isFullScreen())
    else
      PlanckEnv.maximize()
...
class ToolbarMenuControl extends React.Component
  ...
  _openMenu: =>
    ...
    applicationMenu.menu.popup(PlanckEnv.getCurrentWindow())
