...
# Essential: PlanckEnv global for dealing with packages, themes, menus, and the window.
#
# The singleton of this class is always available as the `PlanckEnv` global.
module.exports =
class PlanckEnvConstructor extends Model
  ...
  # Returns an PlanckEnv instance, fully initialized
  @loadOrCreate: ->
    startTime = Date.now()
  ...
  #
  # Call after this instance has been assigned to the `PlanckEnv` global.
  initialize: ->
    ...
    # Set PlanckEnv's home so packages don't have to guess it
  ...
  # `PlanckEnv.reportError` will hook into the event callbacks and handle
  ...
  loadSettingsChanged: (event, loadSettings) =>
    ...
    console.log "PlanckEnv->loadSettingsChanged"
