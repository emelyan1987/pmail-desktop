...
# application menu.
#
# An instance of this class is always available as the `PlanckEnv.menu` global.
#
# ## Menu CSON Format
#
# ```coffee
...
class MenuManager
  constructor: ({@resourcePath}) ->
    ...
    @template = []
    PlanckEnv.keymaps.onDidLoadBundledKeymaps => @loadPlatformItems()
    PlanckEnv.keymaps.onDidReloadKeymap => @update()
    PlanckEnv.packages.onDidActivateInitialPackages => @sortPackagesMenu()
  ...
  update: ->
    ...
    @pendingUpdateOperation = setImmediate =>
      keystrokesByCommand = {}
      for binding in PlanckEnv.keymaps.getKeyBindings() when @includeSelector(binding.selector)
