...
class APMWrapper
  ...
  runCommand: (args, options, callback) ->
    command = PlanckEnv.packages.getApmPath()
    outputLines = []
    ...
    options.env =
      ...
      ATOM_RESOURCE_PATH: PlanckEnv.getLoadSettings().resourcePath
      ATOM_HOME: PlanckEnv.getConfigDirPath()
  ...
  loadCompatiblePackageVersion: (packageName, callback) ->
    args = ['view', packageName, '--json', '--compatible', @normalizeVersion(PlanckEnv.getVersion())]
    errorMessage = "Fetching package '#{packageName}' failed."
  ...
  update: (pack, newVersion, callback) ->
    ...
    if theme
      activateOnSuccess = PlanckEnv.packages.isPackageActive(name)
    else
      activateOnSuccess = not PlanckEnv.packages.isPackageDisabled(name)
    activateOnFailure = PlanckEnv.packages.isPackageActive(name)
    PlanckEnv.packages.deactivatePackage(name) if PlanckEnv.packages.isPackageActive(name)
    PlanckEnv.packages.unloadPackage(name) if PlanckEnv.packages.isPackageLoaded(name)
    ...
    exit = (code, stdout, stderr) =>
      if code is 0
        if activateOnSuccess
          PlanckEnv.packages.activatePackage(name)
        else
          PlanckEnv.packages.loadPackage(name)

        callback?()
      else
        PlanckEnv.packages.activatePackage(name) if activateOnFailure
        error = new Error(errorMessage)
        error.stdout = stdout
        error.stderr = stderr
        onError(error)
  ...
  unload: (packageName) ->
    if PlanckEnv.packages.isPackageLoaded(name)
      PlanckEnv.packages.deactivatePackage(name) if PlanckEnv.packages.isPackageActive(name)
      PlanckEnv.packages.unloadPackage(name)

  install: (pack, callback) ->
    {name, version, theme} = pack
    activateOnSuccess = not theme and not PlanckEnv.packages.isPackageDisabled(name)
    activateOnFailure = PlanckEnv.packages.isPackageActive(name)
