...
class WindowManager
  ...
  showMainWindow: (loadingMessage) ->
    ...
    if @_mainWindow
      ...
      #console.log "showMainWindow ===> " + @_mainWindow
      if @devMode
      ...
      #console.log "bootstrapScript ==> " + bootstrapScript
      resourcePath ?= @resourcePath
      #console.log "resourcePaht ==> " + resourcePath
  ...
  ensureOnboardingWindow: ({welcome, provider}={}) ->
    ...
    if existing
      ...
      if welcome
        options.title = "Welcome to PlanckMail"
  ...
  # add a callback to `PlanckEnv.onWindowPropsChanged`.
  #
  ...
  # loadSettings, which is accessible via `PlanckEnv.getLoadSettings()`
  #
  # REQUIRED options:
  #   - windowType: defaults "popout". This eventually ends up as
  #     PlanckEnv.getWindowType()
  #
  # Valid options:
  #   - coldStart: true
