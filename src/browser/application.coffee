...
#
# It's the entry point into the Planck application and maintains the global state
# of the application.
#
module.exports =
class Application
  ...
  # Public: The entry point into the Planck application.
  @open: (options) ->
  ...
  constructor: (options) ->
    ...
    #console.log "Application.LaunchWithOptions() ==> "
    #console.log options
  ...
  openWindowsForTokenState: (loadingMessage) =>
    ...
    #console.log @config.get('nylas.accounts')
    hasAccount = @config.get('planck.accounts')?.length > 0
    if hasAccount
