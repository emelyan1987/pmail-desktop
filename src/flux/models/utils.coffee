...
Utils =
  ...
  correctTimeString: (time) ->
    return "" unless time
    moment(time).tz(Utils.timeZone).format("MM/DD/YYYY h:mm:ss")
  ...
  imageNamed: (fullname, resourcePath) ->
    ...
    DefaultResourcePath ?= PlanckEnv.getLoadSettings().resourcePath
  ...
  fileSafeDisplayName: (filename)->
    return filename.replace(RegExpUtils.illegalPathCharactersRegexp(), '-')

  fileExtension: (filename)->
    return path.extname(filename.toLowerCase())[1..-1]
