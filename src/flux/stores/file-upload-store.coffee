...
UPLOAD_DIR = path.join(PlanckEnv.getConfigDirPath(), 'uploads')
...
class FileUploadStore extends NylasStore
  ...
  constructor: ->
    ...
    if PlanckEnv.isMainWindow() or PlanckEnv.inSpecMode()
  ...
  _onSelectAttachment: ({messageClientId, provider}) ->
    @_verifyId(messageClientId)
    return if not (provider && provider.type)

    if provider.type=="local"
      # When the dialog closes, it triggers `Actions.addAttachment`
      PlanckEnv.showOpenDialog {properties: ['openFile', 'multiSelections']}, (pathsToOpen) ->
  ...
  _onAddAttachment: ({messageClientId, filePath}) ->
    ...
    .then(@_copyUpload)
    .then (upload) =>
      console.log "FileUploadStore->_onAddAttachment"
      console.log upload
  ...
  _onAttachFileError: (error) ->
    PlanckEnv.showErrorDialog(error.message)
