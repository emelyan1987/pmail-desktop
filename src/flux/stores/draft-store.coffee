...
class DraftStore
  ...
  constructor: ->
    ...
    @listenTo Actions.composeNewDraftWithFileLinks, @_onPopoutDraftWithFileLinks
    @listenTo Actions.composeNewDraftWithFileDownloads, @_onPopoutDraftWithFileDownloads
    ...
    PlanckEnv.onBeforeUnload @_onBeforeUnload
  ...
  _onBeforeUnload: (readyToUnload) =>
    ...
    if promises.length > 0
      ...
      # Firing PlanckEnv.close() does nothing if called within an existing beforeUnload
  ...
  _onPopoutDraftWithFileLinks:(links) =>
    tags = _.map links, (link)=>
      "<a href='#{link}'>#{link}</a>"

    DraftFactory.createDraft({body:tags.join('<br>')}).then (draft) =>
      @_finalizeAndPersistNewMessage(draft).then ({draftClientId}) =>
        @_onPopoutDraftClientId(draftClientId, {newDraft: true})

  _onPopoutDraftWithFileDownloads:(files) =>
    console.log "DraftStore->_onPopoutDraftWithFileDownloads"
    console.log files
    DraftFactory.createDraft({downloads:files}).then (draft) =>
      console.log "After createDraft"
      console.log draft
      @_finalizeAndPersistNewMessage(draft).then ({draftClientId}) =>
        @_onPopoutDraftClientId(draftClientId, {newDraft: true})
  ...
  _isPopout: ->
    PlanckEnv.getWindowType() is "composer"
        ...
        Promise.resolve(extApply({draft: cleaned})).then (transformed) =>
          Promise.resolve(extUnapply({draft: transformed.clone()})).then (untransformed) =>
            ...
            if not _.isEqual(_.pick(untransformed, allowedFields), _.pick(cleaned, allowedFields))
              ...
              PlanckEnv.reportError(new Error("An extension applied a tranform to the draft that it could not reverse."))
  ...
  _notifyUserOfError: ({draftClientId, threadId, errorMessage}) ->
    ...
    if threadId and focusedThread?.id is threadId
      PlanckEnv.showErrorDialog(errorMessage)
    else
