...
configAccountsKey = "planck.accounts"
configVersionKey = "planck.accountsVersion"
configTokensKey = "planck.accountTokens"
keytarServiceName = 'Planck'
...
class AccountStore extends NylasStore
  ...
  constructor: ->
    ...
    PlanckEnv.config.onDidChange configVersionKey, (change) =>
  ...
  _loadAccounts: =>
    @_caches = {}
    @_version = PlanckEnv.config.get(configVersionKey) || 0

    @_accounts = []
    for json in PlanckEnv.config.get(configAccountsKey) || []
      @_accounts.push((new Account).fromJSON(json))

    oldTokens = PlanckEnv.config.get(configTokensKey)
    if oldTokens
  ...
  _save: =>
    @_version += 1
    PlanckEnv.config.set(configVersionKey, @_version)
    PlanckEnv.config.set(configAccountsKey, @_accounts)
    PlanckEnv.config.set(configTokensKey, null)
  ...
  addAccountFromJSON: (json) =>
    console.log "AccountStore->addAccountFromJSON", json
    ...
    @_tokens[json.id] = json.access_token
    keytar.replacePassword(keytarServiceName, json.email_address, json.access_token)
  ...
  _importFakeData: (dir) =>
    ...
    if fs.existsSync(downloadsDir)
      for filename in fs.readdirSync(downloadsDir)
        fs.copySync(path.join(downloadsDir, filename), path.join(PlanckEnv.getConfigDirPath(), 'downloads', filename))
