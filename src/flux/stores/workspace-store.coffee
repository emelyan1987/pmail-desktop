...
more about sheets and layout in PlanckMail, see the {InterfaceConcepts.md}
documentation.
...
class WorkspaceStore extends NylasStore
  constructor: ->
    @_resetInstanceVars()
    @_preferredLayoutMode = PlanckEnv.config.get('core.workspace.mode')
    ...
    {windowType} = PlanckEnv.getLoadSettings()
    unless windowType is 'onboarding'
      require('electron').webFrame.setZoomLevelLimits(1, 1)
      PlanckEnv.config.observe 'core.workspace.interfaceZoom', (z) =>
        require('electron').webFrame.setZoomFactor(z) if z and _.isNumber(z)

    PlanckEnv.commands.add 'body', @_navigationCommands()
  ...
  _selectViewMode: (mode) =>
    ...
    PlanckEnv.config.set('core.workspace.mode', @_preferredLayoutMode)
    @popToRootSheet()
    @trigger()
  ...
  _resetInstanceVars: =>
    ...
    @_hiddenLocations = PlanckEnv.config.get('core.workspace.hiddenLocations') || {}
    @_sheetStack = []
  ...
  _onToggleLocationHidden: (location) =>
    ...
    PlanckEnv.config.set('core.workspace.hiddenLocations', @_hiddenLocations)
  ...
  pushSheet: (sheet) =>
    @_sheetStack.push(sheet)
    console.log "SheetStack"
    console.log @_sheetStack
    @trigger()
