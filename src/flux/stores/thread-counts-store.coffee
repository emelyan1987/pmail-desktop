...
class ThreadCountsStore extends NylasStore
  ...
  constructor: ->
    ...
    if PlanckEnv.isWorkWindow() and not PlanckEnv.config.get('nylas.threadCountsValid')
      @reset()
  ...
  reset: =>
    ...
    .then =>
      PlanckEnv.config.set('nylas.threadCountsValid', true)
  ...
  _onCountsChanged: =>
    DatabaseStore._query(ReadCountsQuery()).then (results) =>
      ...
      if foundNegative
        PlanckEnv.reportError(new Error('Assertion Failure: Negative Count'))
        @reset()
        return
