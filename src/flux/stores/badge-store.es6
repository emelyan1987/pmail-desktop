...
class BadgeStore extends NylasStore {
  ...
  constructor() {
    ...
    PlanckEnv.config.onDidChange('core.notifications.unreadBadge', ({newValue}) => {
      if (newValue === true) {
  ...
  _setBadgeForCount = () => {
    if (!PlanckEnv.config.get('core.notifications.unreadBadge')) {
      return;
    }
    if (!PlanckEnv.isMainWindow() && !PlanckEnv.inSpecMode()) {
      return;
    }
