...
DEBUG_TO_LOG = false
DEBUG_QUERY_PLANS = PlanckEnv.inDevMode()
...
class DatabaseStore extends NylasStore
  ...
  constructor: ->
    ...
    if PlanckEnv.inSpecMode()
      @_databasePath = path.join(PlanckEnv.getConfigDirPath(),'edgehill.test.db')
    else
      @_databasePath = path.join(PlanckEnv.getConfigDirPath(),'edgehill.db')

    console.log "Database Path"+@_databasePath
  ...
  _onPhaseChange: (event) =>
    return if PlanckEnv.inSpecMode()

    app = remote.getGlobal('application')
    phase = app.databasePhase()

    if phase is DatabasePhase.Setup and PlanckEnv.isWorkWindow()
      @_openDatabase =>
  ...
  refreshDatabaseSchema: ->
    return unless PlanckEnv.isWorkWindow()
    app = remote.getGlobal('application')
    phase = app.databasePhase()
  ...
  _runDatabaseSetup: (ready) =>
    ...
    @_db.serialize =>
      ...
      , (err) =>
        ...
        # Add custom PlanckContacts Table Create Part Start

        @_db.run "CREATE TABLE IF NOT EXISTS `PlanckContacts` (
        ...
        # Add custom PlanckContacts Table Create Part End
          exportPath = path.join(PlanckEnv.getConfigDirPath(), 'mail-rules-export.json')
  ...
  # cutsom get example for planck DatabaseStore entity
  getplanckdb: =>
    app = remote.getGlobal('application')
    phase = app.databasePhase()
    return @_db if phase is DatabasePhase.Ready
  # cutsom get example for planck DatabaseStore entity
  findJSONBlob: (id) ->
