...
class NylasAPIRequest
  ...
  run: ->
    ...
    console.log "NylasAPI->run", JSON.stringify(@options), token
      ...
      req = request @options, (error, response, body) =>
        ...
        PriorityUICoordinator.settle.then =>
          if error or response.statusCode > 299
            ...
            PlanckEnv.errorLogger.apiDebug(apiError)
...
class NylasAPI
  ...
  constructor: ->
    ...
    PlanckEnv.config.onDidChange('env', @_onConfigChanged)
    @_onConfigChanged()

  _onConfigChanged: =>
    prev = {@AppID, @APIRoot, @APITokens}

    if PlanckEnv.inSpecMode()
      @pluginsSupported = true
      env = "testing"
    else
      env = PlanckEnv.config.get('env')
    ...
    if env in ['production']
      @AppID = '5girg6tjmjuenujbsg0lnatlq'
      #@AppID = 'eco3rpsghu81xdc48t5qugwq7'
      @AppSecret = '8fokx1yoht10ypwdgev3rqqlp'
      ...
      @AppID = PlanckEnv.config.get('syncEngine.AppID') or 'n/a'
      @APIRoot = 'http://localhost:5555'
    else if env in ['custom']
      @AppID = PlanckEnv.config.get('syncEngine.AppID') or 'n/a'
      @APIRoot = PlanckEnv.config.get('syncEngine.APIRoot') or 'http://localhost:5555'

    current = {@AppID, @APIRoot, @APITokens}

    console.log "NylasAPI->onConfigChanged", env, @AppID, @APIRoot
  ...
  #
  makeRequest: (options={}) ->
    if PlanckEnv.getLoadSettings().isSpec
    ...
    error = (err) =>
      ...
      if err.response
        ...
        if err.response.statusCode in [401, 403]
          ...
          handlePromise = @_handleAuthenticationFailure(options.url, options.auth?.user, options.body, err.response.statusCode)
  ...
  _handleAuthenticationFailure: (modelUrl, apiToken, body, errorCode) ->
    # prevent /auth errors from presenting auth failure notices

    return Promise.resolve() unless apiToken
    #alert("NylasAPI->_handleAuthenticationFailure"+JSON.stringify({url:modelUrl,body:body,token:apiToken,errorCode:errorCode}))
  ...
  getFiles: (accountId, callback) ->
    @makeRequest
      path: "/files"
      accountId: accountId
      success: (body)=>
        console.log "NylasAPI->getFiles"
        console.log body
  ...
  authPlugin: (pluginId, pluginName, accountOrId) ->
    ...
    if PlanckEnv.config.get(cacheKey)
      return Promise.resolve()
    ...
    return @makeRequest({
      ...
      if result.authed
        PlanckEnv.config.set(cacheKey, Date.now())
        return Promise.resolve()
