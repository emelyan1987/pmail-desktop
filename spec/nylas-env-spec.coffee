...
describe "the `PlanckEnv` global", ->
  describe 'window sizing methods', ->
    describe '::getPosition and ::setPosition', ->
      it 'sets the position of the window, and can retrieve the position just set', ->
        PlanckEnv.setPosition(22, 45)
        expect(PlanckEnv.getPosition()).toEqual x: 22, y: 45

    describe '::getSize and ::setSize', ->
      originalSize = null
      beforeEach ->
        originalSize = PlanckEnv.getSize()
      afterEach ->
        PlanckEnv.setSize(originalSize.width, originalSize.height)

      it 'sets the size of the window, and can retrieve the size just set', ->
        PlanckEnv.setSize(100, 400)
        expect(PlanckEnv.getSize()).toEqual width: 100, height: 400

    describe '::setMinimumWidth', ->
      win = PlanckEnv.getCurrentWindow()

      it "sets the minimum width", ->
        inputMinWidth = 500
        win.setMinimumSize(1000, 1000)

        PlanckEnv.setMinimumWidth(inputMinWidth)
    ...
    describe '::getDefaultWindowDimensions', ->
      it "returns primary display's work area size if it's small enough", ->
        ...
        out = PlanckEnv.getDefaultWindowDimensions()
  ...
  describe ".isReleasedVersion()", ->
    it "returns false if the version is a SHA and true otherwise", ->
      version = '0.1.0'
      spyOn(PlanckEnv, 'getVersion').andCallFake -> version
      expect(PlanckEnv.isReleasedVersion()).toBe true
      version = '36b5518'
      expect(PlanckEnv.isReleasedVersion()).toBe false
  ...
  xdescribe "when an update becomes available", ->
    ...
    it "invokes onUpdateAvailable listeners", ->
      ...
      subscription = PlanckEnv.onUpdateAvailable updateAvailableHandler
  ...
  xdescribe "loading default config", ->
    it 'loads the default core config', ->
      expect(PlanckEnv.config.get('core.excludeVcsIgnoredPaths')).toBe true
      expect(PlanckEnv.config.get('core.followSymlinks')).toBe false
      expect(PlanckEnv.config.get('editor.showInvisibles')).toBe false

  xdescribe "window onerror handler", ->
    beforeEach ->
      spyOn PlanckEnv, 'openDevTools'
      spyOn PlanckEnv, 'executeJavaScriptInDevTools'
    ...
    it "will open the dev tools when an error is triggered", ->
      ...
      expect(PlanckEnv.openDevTools).toHaveBeenCalled()
      expect(PlanckEnv.executeJavaScriptInDevTools).toHaveBeenCalled()
    ...
    describe "::onWillThrowError", ->
      ...
      it "is called when there is an error", ->
        error = null
        PlanckEnv.onWillThrowError(willThrowSpy)
        try
          a + 1
        catch e
          error = e
      ...
      it "will not show the devtools when preventDefault() is called", ->
        ...
        expect(PlanckEnv.openDevTools).not.toHaveBeenCalled()
        expect(PlanckEnv.executeJavaScriptInDevTools).not.toHaveBeenCalled()
    ...
    describe "::onDidThrowError", ->
      ...
      it "is called when there is an error", ->
        error = null
        PlanckEnv.onDidThrowError(didThrowSpy)
        try
          a + 1
        catch e
          error = e
