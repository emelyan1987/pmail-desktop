...
describe "ThemeManager", ->
  themeManager = null
  resourcePath = PlanckEnv.getLoadSettings().resourcePath
  configDirPath = PlanckEnv.getConfigDirPath()
  ...
  beforeEach ->
    ...
    spyOn(PlanckEnv.packages, "getAvailablePackagePaths").andReturn packagePaths
    PlanckEnv.packages.packageDirPaths.unshift(theme_dir)
    themeManager = new ThemeManager({packageManager: PlanckEnv.packages, resourcePath, configDirPath})
  ...
  describe "theme getters and setters", ->
    ...
    it 'getActiveThemes get all the active themes', ->
      ...
      runs ->
        names = PlanckEnv.config.get('core.themes')
  ...
  describe "when the core.themes config value contains invalid entry", ->
    it "ignores theme", ->
      PlanckEnv.config.set 'core.themes', [
        'ui-light'
        null
        undefined
        ''
  ...
  describe "::getImportPaths()", ->
    it "returns the theme directories before the themes are loaded", ->
      PlanckEnv.config.set('core.themes', ['theme-with-index-less', 'ui-dark', 'ui-light'])
    ...
    it "ignores themes that cannot be resolved to a directory", ->
      PlanckEnv.config.set('core.themes', ['definitely-not-a-theme'])
  ...
  describe "when the core.themes config value changes", ->
    it "add/removes stylesheets to reflect the new config value", ->
      ...
      spyOn(PlanckEnv.styles, 'getUserStyleSheetPath').andCallFake -> null
      ...
      runs ->
        ...
        PlanckEnv.config.set('core.themes', ['ui-dark'])
      ...
      runs ->
        ...
        PlanckEnv.config.set('core.themes', ['ui-light', 'ui-dark'])
      ...
      runs ->
        ...
        PlanckEnv.config.set('core.themes', ['theme-with-index-less', 'ui-light'])
    ...
    it 'adds theme-* classes to the workspace for each active theme', ->
      ...
      runs ->
        ...
        themeManager.onDidChangeActiveThemes didChangeActiveThemesHandler = jasmine.createSpy()
        PlanckEnv.config.set('core.themes', ['theme-with-ui-variables'])
  ...
  describe "when a theme fails to load", ->
    it "logs a warning", ->
      PlanckEnv.packages.activatePackage('a-theme-that-will-not-be-found')
  ...
  describe "::requireStylesheet(path)", ->
    ...
    it "synchronously loads css at the given path and installs a style tag for it in the head", ->
      PlanckEnv.styles.onDidAddStyleElement styleElementAddedHandler = jasmine.createSpy("styleElementAddedHandler")
    ...
    it "returns a disposable allowing styles applied by the given path to be removed", ->
      ...
      PlanckEnv.styles.onDidRemoveStyleElement styleElementRemovedHandler = jasmine.createSpy("styleElementRemovedHandler")
  ...
  describe "base style sheet loading", ->
    ...
    describe "when there is a theme with incomplete variables", ->
      it "loads the correct values from the fallback ui-variables", ->
        PlanckEnv.config.set('core.themes', ['theme-with-incomplete-ui-variables'])
  ...
  describe "user stylesheet", ->
    ...
    beforeEach ->
      ...
      spyOn(PlanckEnv.styles, 'getUserStyleSheetPath').andReturn userStylesheetPath
    ...
    describe "when there is an error reading the stylesheet", ->
      ...
      it "creates an error notification and does not add the stylesheet", ->
        ...
        expect(PlanckEnv.styles.styleElementsBySourcePath[PlanckEnv.styles.getUserStyleSheetPath()]).toBeUndefined()
  ...
  describe "when a non-existent theme is present in the config", ->
    beforeEach ->
      PlanckEnv.config.set('core.themes', ['non-existent-dark-ui'])
  ...
  describe "when in safe mode", ->
    beforeEach ->
      themeManager = new ThemeManager({packageManager: PlanckEnv.packages, resourcePath, configDirPath, safeMode: true})

    describe 'when the enabled UI theme is bundled with N1', ->
      beforeEach ->
        PlanckEnv.config.set('core.themes', ['ui-light'])
    ...
    describe 'when the enabled UI theme is not bundled with N1', ->
      beforeEach ->
        PlanckEnv.config.set('core.themes', ['installed-dark-ui'])
