...
describe "AccountStore", ->
  ...
  describe "adding account from json", ->
    ...
    it "saves the account to the accounts cache and saves", ->
      ...
      expect(PlanckEnv.config.set.calls.length).toBe 3
      expect(PlanckEnv.config.set.calls[2].args).toEqual(['nylas.accountTokens', null])
      expect(PlanckEnv.config.save).toHaveBeenCalled()
