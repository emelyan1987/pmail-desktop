...
describe "FileDownloadStore", ->
  ...
  describe "_newMailReceived", ->
    it "should fetch attachments if the setting is on-receive", ->
      ...
      spyOn(PlanckEnv.config, 'get').andCallFake (key) ->
  ...
  describe "_fetchAndSave", ->
    beforeEach ->
      ...
      spyOn(PlanckEnv, 'showSaveDialog').andCallFake (options, callback) => callback(@userSelectedPath)
    ...
    it "should open a save dialog and prompt the user to choose a download path", ->
      ...
      expect(PlanckEnv.showSaveDialog).toHaveBeenCalled()
    ...
    describe "when the user confirms a path", ->
      ...
      it "should update the PlanckEnv.savedState.lastDownloadDirectory", ->
        PlanckEnv.savedState.lastDownloadDirectory = null
        @userSelectedPath = "/Users/imaginary/.nylas/Another Random Folder/file.jpg"
        FileDownloadStore._fetchAndSave(@testfile)
        advanceClock(1)
        expect(PlanckEnv.savedState.lastDownloadDirectory).toEqual('/Users/imaginary/.nylas/Another Random Folder')
