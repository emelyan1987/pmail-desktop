...
describe "StylesElement", ->
  ...
  it "renders a style tag for all currently active stylesheets in the style manager", ->
    ...
    disposable1 = PlanckEnv.styles.addStyleSheet("a {color: red;}")
    ...
    disposable2 = PlanckEnv.styles.addStyleSheet("a {color: blue;}")
  ...
  it "orders style elements by priority", ->
    ...
    PlanckEnv.styles.addStyleSheet("a {color: red}", priority: 1)
    PlanckEnv.styles.addStyleSheet("a {color: blue}", priority: 0)
    PlanckEnv.styles.addStyleSheet("a {color: green}", priority: 2)
    PlanckEnv.styles.addStyleSheet("a {color: yellow}", priority: 1)
  ...
  it "updates existing style nodes when style elements are updated", ->
    ...
    PlanckEnv.styles.addStyleSheet("a {color: red;}", sourcePath: '/foo/bar')
    PlanckEnv.styles.addStyleSheet("a {color: blue;}", sourcePath: '/foo/bar')
  ...
  it "only includes style elements matching the 'context' attribute", ->
    ...
    PlanckEnv.styles.addStyleSheet("a {color: red;}", context: 'test-context')
    PlanckEnv.styles.addStyleSheet("a {color: green;}")
    ...
    PlanckEnv.styles.addStyleSheet("a {color: blue;}", context: 'test-context')
    PlanckEnv.styles.addStyleSheet("a {color: yellow;}")
  ...
  describe "nylas-theme-wrap shadow DOM selector upgrades", ->
    ...
    it "upgrades selectors containing .editor-colors", ->
      PlanckEnv.styles.addStyleSheet(".editor-colors {background: black;}", context: 'nylas-theme-wrap')
      expect(element.firstChild.sheet.cssRules[0].selectorText).toBe ':host'

    it "upgrades selectors containing .editor", ->
      PlanckEnv.styles.addStyleSheet """
    ...
    it "defers selector upgrade until the element is attached", ->
      ...
      PlanckEnv.styles.addStyleSheet ".editor {background: black;}", context: 'nylas-theme-wrap'
