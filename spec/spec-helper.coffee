...
require '../src/window'
PlanckEnv.restoreWindowDimensions()
...
PlanckEnv.themes.loadBaseStylesheets()
PlanckEnv.themes.requireStylesheet '../static/jasmine'
PlanckEnv.themes.initialLoadComplete = true

PlanckEnv.keymaps.loadBundledKeymaps()
keyBindingsToRestore = PlanckEnv.keymaps.getKeyBindings()
commandsToRestore = PlanckEnv.commands.getSnapshot()
styleElementsToRestore = PlanckEnv.styles.getSnapshot()

window.addEventListener 'core:close', -> window.close()
window.addEventListener 'beforeunload', ->
  PlanckEnv.storeWindowDimensions()
  PlanckEnv.saveSync()
...
{specDirectory, resourcePath} = PlanckEnv.getLoadSettings()
...
beforeEach ->
  PlanckEnv.testOrganizationUnit = null
  ...
  # false because we never setup the DB when `PlanckEnv.inSpecMode` is
  # true.
  ...
  $.fx.off = true
  documentTitle = null
  PlanckEnv.packages.serviceHub = new ServiceHub
  PlanckEnv.keymaps.keyBindings = _.clone(keyBindingsToRestore)
  PlanckEnv.commands.restoreSnapshot(commandsToRestore)
  PlanckEnv.styles.restoreSnapshot(styleElementsToRestore)
  PlanckEnv.workspaceViewParentSelector = '#jasmine-content'

  PlanckEnv.packages.packageStates = {}

  serializedWindowState = null

  spyOn(PlanckEnv, 'saveSync')

  TimeOverride.resetTime()
  TimeOverride.enableSpies()

  spy = spyOn(PlanckEnv.packages, 'resolvePackagePath').andCallFake (packageName) ->
    if specPackageName and packageName is specPackageName
      resolvePackagePath(specPackagePath)
    else
      resolvePackagePath(packageName)
  resolvePackagePath = _.bind(spy.originalValue, PlanckEnv.packages)

  # prevent specs from modifying N1's menus
  spyOn(PlanckEnv.menu, 'sendToBrowserProcess')
  ...
  AccountStore._accounts = [
    new Account({
      ...
      organizationUnit: PlanckEnv.testOrganizationUnit || 'label'
  ...
  spyOn(Config::, 'load')
  spyOn(Config::, 'save')
  config = new Config({resourcePath, configDirPath: PlanckEnv.getConfigDirPath()})
  PlanckEnv.config = config
  PlanckEnv.loadConfig()
...
afterEach ->
  ...
  PlanckEnv.themes.removeStylesheet('global-editor-styles')

  delete PlanckEnv.state?.packageStates

  $('#jasmine-content').empty() unless window.debugContent

  ReactTestUtils.unmountAll()

  jasmine.unspy(PlanckEnv, 'saveSync')
