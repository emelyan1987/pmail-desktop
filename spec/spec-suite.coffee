...
requireSpecs = (specDirectory) ->
  specFilePattern = PlanckEnv.getLoadSettings().specFilePattern
...
runAllSpecs = ->
  {resourcePath} = PlanckEnv.getLoadSettings()
  ...
  # packagePaths = PlanckEnv.packages.getAvailablePackageNames().map (packageName) ->
  #   PlanckEnv.packages.resolvePackagePath(packageName)
...
if specDirectory = PlanckEnv.getLoadSettings().specDirectory
  requireSpecs(specDirectory)
  setSpecType('user')
else
  runAllSpecs()
