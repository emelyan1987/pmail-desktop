...
describe "when we open a file", ->
  beforeEach ->
    waitsForPromise ->
      PlanckEnv.workspace.open 'c.coffee'

  it "should be opened in an editor", ->
    expect(PlanckEnv.workspace.getActiveTextEditor().getPath()).toContain 'c.coffee'
...
describe "waiting for the packages to load", ->
  ...
  beforeEach ->
    waitsForPromise ->
      PlanckEnv.workspace.open('sample.js')
    waitsForPromise ->
      PlanckEnv.packages.activatePackage('tabs')
    waitsForPromise ->
      PlanckEnv.packages.activatePackage('tree-view')

  it 'should have waited long enough', ->
    expect(PlanckEnv.packages.isPackageActive('tabs')).toBe true
    expect(PlanckEnv.packages.isPackageActive('tree-view')).toBe true
```
