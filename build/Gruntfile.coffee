...
# shellAppDir = /tmp/nylas-build/PlanckMail.app
# contentsDir = /tmp/nylas-build/PlanckMail.app/Contents
# appDir      = /tmp/nylas-build/PlanckMail.app/Contents/Resources/app
#
# installDir = /Applications/PlanckMail.app
#
# And on Linux:
#
# tmpDir: /tmp/
...
module.exports = (grunt) ->
  ...
  tmpDir = os.tmpdir()
  appName = if process.platform is 'darwin' then 'PlanckMail.app' else 'PlanckMail'
  ...
  electronDownloadDir = path.join(home, '.planck', 'electron')
  ...
  if process.platform is 'win32'
    ...
    killCommand = 'taskkill /F /IM plnack.exe'
  else if process.platform is 'darwin'
    contentsDir = path.join(shellAppDir, 'Contents')
    appDir = path.join(contentsDir, 'Resources', 'app')
    installDir ?= path.join('/Applications', appName)
    killCommand = 'pkill -9 Planck'
  else
    contentsDir = shellAppDir
    appDir = path.join(shellAppDir, 'resources', 'app')
  ...
  csonConfig =
    options:
      rootObject: false
      cachePath: path.join(home, '.planck', 'compile-cache', 'grunt-cson')
