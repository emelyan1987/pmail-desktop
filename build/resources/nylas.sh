...
if [ $OS == 'Mac' ]; then
  NYLAS_APP_NAME="PlanckMail.app"

  if [ -z "${NYLAS_PATH}" ]; then
    # If NYLAS_PATH isnt set, check /Applications and then ~/Applications for PlanckMail.app
    if [ -x "/Applications/$NYLAS_APP_NAME" ]; then
      ...
      # We havent found an PlanckMail.app, use spotlight to search for N1
      ...
      if [ ! -x "$NYLAS_PATH/$NYLAS_APP_NAME" ]; then
        echo "Cannot locate 'PlanckMail.app', it is usually located in /Applications. Set the NYLAS_PATH environment variable to the directory containing 'PlanckMail.app'."
        exit 1
      fi
    fi
  fi
  ...
  if [ $EXPECT_OUTPUT ]; then
    "$NYLAS_PATH/$NYLAS_APP_NAME/Contents/MacOS/Planck" --executed-from="$(pwd)" --pid=$$ "$@"
    exit $?
  else
