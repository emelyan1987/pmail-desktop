...
module.exports =
  ...
  activate: (@state) ->
    ...
    configVersion = PlanckEnv.config.get("lastVersion")
    currentVersion = PlanckEnv.getVersion()
    if configVersion and configVersion isnt currentVersion
      PlanckEnv.config.set("lastVersion", currentVersion)
      @displayThanksNotification()

    if updater.getState() is 'update-available'
      @displayNotification(updater.releaseVersion)

    PlanckEnv.onUpdateAvailable ({releaseVersion, releaseNotes} = {}) =>
