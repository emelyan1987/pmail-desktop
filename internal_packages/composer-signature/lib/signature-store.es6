...
class SignatureStore {
  ...
  DefaultSignature = "Sent from <a href=\"http://www.plancklabs.com\">PlanckLabs</a>, the extensible, open source mail client.";
  ...
  signatureForAccountId(accountId) {
    ...
    const saved = PlanckEnv.config.get(`nylas.account-${accountId}.signature`);
    if (saved === undefined) {
      return this.DefaultSignature;
    }
    return saved;
  ...
  _onSetSignatureForAccountId = ({signature, accountId}) => {
    // PlanckEnv.config.set is internally debounced 100ms
    PlanckEnv.config.set(`nylas.account-${accountId}.signature`, signature)
  }
}
