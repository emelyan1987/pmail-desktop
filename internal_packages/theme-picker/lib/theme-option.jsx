...
class ThemeOption extends React.Component {
  ...
  _getImportPaths() {
    ...
    for (const theme of PlanckEnv.themes.getActiveThemes()) {
      if (theme.name === PlanckEnv.themes.baseThemeName()) {
        themes.push(theme);
      }
    }
    const themePaths = [];
  ...
  _loadLessStylesheet(lessStylesheetPath) {
    const {configDirPath, resourcePath} = PlanckEnv.getLoadSettings();
    if (this.lessCache) {
