...
module.exports =
  activate: (@state) ->
    ...
    return if PlanckEnv.config.get(NOTIF_SETTINGS_KEY) is true

    @services.isRegisteredForURLScheme 'mailto', (registered) =>
      # Prompt them to make PlanckMail their default client
      unless registered
        @_unlisten = Actions.notificationActionTaken.listen(@_onNotificationActionTaken, @)
        Actions.postNotification
          type: 'info',
          sticky: true
          message: "Thanks for trying out PlanckMail! Would you like to make it your default mail client?",
          icon: 'fa-inbox',
          actions: [{
            label: 'Not Now'
            dismisses: true
  ...
  _onNotificationActionTaken: ({notification, action}) ->
    ...
    if action.id is NOTIF_ACTION_NO
      PlanckEnv.config.set(NOTIF_SETTINGS_KEY, true)
