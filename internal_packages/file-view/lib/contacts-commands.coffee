...
class ContactsCommands
  ...
  @_focusAccounts: (accounts) ->
    ...
    PlanckEnv.show() unless PlanckEnv.isVisible()
  ...
  @registerCommands: (accounts) ->
    ...
    @_commandsDisposable = PlanckEnv.commands.add('body', commands)

  @registerMenuItems: (accounts, focusedAccounts) ->
    windowMenu = _.find PlanckEnv.menu.template, ({label}) ->
...
module.exports = ContactsCommands
