...
const PackagesStore = Reflux.createStore({
  ...
  init: function init() {
    ...
    this.listenTo(PluginsActions.showPackage, (pkg) => {
      const dir = PlanckEnv.packages.resolvePackagePath(pkg.name);
    ...
    this.listenTo(PluginsActions.installPackage, (pkg) => {
      ...
      this._apm.install(pkg, (err) => {
        if (err) {
          ...
          if (PlanckEnv.packages.isPackageDisabled(pkg.name)) {
            PlanckEnv.packages.enablePackage(pkg.name);
          }
        }
    ...
    this.listenTo(PluginsActions.uninstallPackage, (pkg) => {
      if (PlanckEnv.packages.isPackageLoaded(pkg.name)) {
        PlanckEnv.packages.disablePackage(pkg.name);
        PlanckEnv.packages.unloadPackage(pkg.name);
      }
  ...
  _prepareIfFresh: function _prepareIfFresh() {
    ...
    PlanckEnv.packages.onDidActivatePackage(() => this._onPackagesChangedDebounced());
    PlanckEnv.packages.onDidDeactivatePackage(() => this._onPackagesChangedDebounced());
    PlanckEnv.packages.onDidLoadPackage(() => this._onPackagesChangedDebounced());
    PlanckEnv.packages.onDidUnloadPackage(() => this._onPackagesChangedDebounced());
    this._onPackagesChanged();
  ...
  _onPackagesChanged: function _onPackagesChanged() {
    ...
    .then((packages) => {
      ...
      const available = PlanckEnv.packages.getAvailablePackageMetadata();
  ...
  _onInstallPackage: function _onInstallPackage() {
    PlanckEnv.showOpenDialog({
      title: "Choose a Plugin Directory",
      properties: ['openDirectory'],
    },
    (filenames) => {
      if (!filenames || filenames.length === 0) return;
      PlanckEnv.packages.installPackageFromPath(filenames[0], (err) => {
        if (err) return;
  ...
  _onCreatePackage: function _onCreatePackage() {
    ...
    const packagesDir = path.join(PlanckEnv.getConfigDirPath(), 'dev', 'packages');
    fs.makeTreeSync(packagesDir);

    PlanckEnv.showSaveDialog({
      ...
      if (PlanckEnv.packages.resolvePackagePath(packageName)) {
      ...
      fs.mkdir(packageDir, (err) => {
        ...
        const {resourcePath} = PlanckEnv.getLoadSettings();
        ...
        const packageJSON = {
          ...
          engines: {
            nylas: `>=${PlanckEnv.getVersion().split('-')[0]}`,
          },
          windowTypes: {
            default: true,
            composer: true,
        ...
        _.defer(() => {
          PlanckEnv.packages.enablePackage(packageDir);
  ...
  _addPackageStates: function _addPackageStates(pkgs) {
    ...
    _.flatten(_.values(pkgs)).forEach((pkg) => {
      pkg.enabled = !PlanckEnv.packages.isPackageDisabled(pkg.name);
