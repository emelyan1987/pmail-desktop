...
countForItem = (perspective) ->
  unreadCountEnabled = PlanckEnv.config.get('core.workspace.showUnreadForAllCategories')
...
isItemCollapsed = (id) ->
  if PlanckEnv.savedState.sidebarKeysCollapsed[id] isnt undefined
    PlanckEnv.savedState.sidebarKeysCollapsed[id]
  else
    true
