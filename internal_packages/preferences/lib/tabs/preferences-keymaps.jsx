...
class PreferencesKeymaps extends React.Component {
  ...
  componentDidMount() {
    this._disposable = PlanckEnv.keymaps.onDidReloadKeymap(() => {
  ...
  _getStateFromKeymaps() {
    ...
    for (const section of displayedKeybindings) {
      for (const [command] of section.items) {
        const keyBinding = PlanckEnv.keymaps.findKeyBindings({command: command});
  ...
  _loadTemplates() {
    const templatesDir = path.join(PlanckEnv.getLoadSettings().resourcePath, 'keymaps', 'templates');
  ...
  _onShowUserKeymaps() {
    const keymapsFile = PlanckEnv.keymaps.getUserKeymapPath();
