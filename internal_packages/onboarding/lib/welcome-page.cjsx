...
class WelcomePage extends React.Component
  ...
  _renderStep0: ->
    <div className={@_stepClass(0)} key="step-0">
      <RetinaImg className="logo" style={marginTop: 86} url="nylas://onboarding/assets/planck-logo@2x.png" mode={RetinaImg.Mode.ContentPreserve}/>
      <p className="hero-text" style={fontSize: 46, marginTop: 57}>Welcome to PlanckMail</p>
     </div>
  ...
  _renderStep1: ->
    <div className={@_stepClass(1)} key="step-1">
      ...
      <p className="sub-text">PlanckMail and the cloud sync engine are available on <a onClick={=> @_open("https://github.com/nylas/n1")}>GitHub</a></p>
      {@_renderNavBubble(1)}
    </div>
  ...
  _onContinue: =>
    @state.step = 2 # addition by emelyan.a
    if @state.step < 2
      @setState(step: @state.step + 1)
    else
