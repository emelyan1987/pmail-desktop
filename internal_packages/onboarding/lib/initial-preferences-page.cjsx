...
class InitialPreferencesOptions extends React.Component
  ...
  _loadTemplates: =>
    templatesDir = path.join(PlanckEnv.getLoadSettings().resourcePath, 'keymaps', 'templates')
...
class InitialPreferencesPage extends React.Component
  ...
  render: =>
    ...
    <div className="page opaque" style={width:900, height:620}>
      <h1 style={paddingTop: 100}>Welcome to PlanckMail</h1>
      <h4 style={marginBottom: 70}>Let's set things up to your liking.</h4>
      <ConfigPropContainer>
        <InitialPreferencesOptions account={account} />
      </ConfigPropContainer>
      <button className="btn btn-large" style={marginBottom:60} onClick={@_onGetStarted}>Start using PlanckMail</button>
    </div>
