...
class InstallButton extends React.Component
  constructor: (@props) ->
    @state =
      installed: !PlanckEnv.packages.isPackageDisabled(@props.package.name)
  ...
  _onInstall: =>
    PlanckEnv.packages.enablePackage(@props.package.name)
...
class InitialPackagesPage extends React.Component
  ...
  constructor: (@props) ->
    @state =
      packages: PlanckEnv.packages.getAvailablePackageMetadata().filter ({isStarterPackage}) => isStarterPackage
