...
class AccountSettingsPage extends React.Component
  ...
  constructor: (@props) ->
    ...
    if @state.provider.name is 'gmail'
      Actions.googleAuthenticationSucceeded.listen(@_onGoogleAuthenticationSucceeded)
  ...
  _onGoogleAuthenticationSucceeded: (params) =>
    @google =
    ...
    options =
        method: "GET"
        url: "https://www.googleapis.com/oauth2/v1/userinfo?alt=json&access_token=#{@google.access_token}"
        json: true
  ...
  _onSubmit: (event) =>
    ...
    data.client_id = NylasAPI.AppID

    if data.provider=='gmail'
        return unless @google

        #alert JSON.stringify(@google)
        data.settings.google_client_id = '65711481506-e1cbv4cci49vacn58n9ujdp4j4hha9pp.apps.googleusercontent.com'
        data.settings.google_client_secret = 'flrAqEGZTdJNn1LRaFVP5Nng'
        data.settings.google_refresh_token = @google.refresh_token

        data.name = @google.name
    ...
    data.reauth_account_id = account.id if account
  ...
  _onNetworkError: (err) =>
    #alert "AccountSettingsPage->_onSubmit->request Error"
