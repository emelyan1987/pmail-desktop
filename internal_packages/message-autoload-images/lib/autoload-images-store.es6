...
class AutoloadImagesStore extends NylasStore {
  ...
  constructor() {
    ...
    this._whitelistEmailsPath = path.join(PlanckEnv.getConfigDirPath(), filename);
    ...
    PlanckEnv.config.onDidChange('core.reading.autoloadImages', () => {
      MessageBodyProcessor.resetCache();
    });
  }

  shouldBlockImagesIn = (message) => {
    if (PlanckEnv.config.get('core.reading.autoloadImages') === true) {
      return false;
    }
