...
class ComposerWithWindowProps extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.state = PlanckEnv.getWindowProps()
  }
  ...
  componentDidMount() {
    ...
    this.unlisten = PlanckEnv.onWindowPropsReceived((windowProps) => {
  ...
  ready = () => {
    this.refs.composer.focus().then(() => {
      ...
      PlanckEnv.getCurrentWindow().focus()
    });
  }
...
export function deactivate() {
  if (PlanckEnv.isMainWindow()) {
    PlanckEnv.unregisterHotWindow('composer');
  }
