...
import AttachActionButton from './attach-action-button';
...
export default class ComposerView extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      downloads: FileDownloadStore.downloadDataForFiles(_.pluck(this.props.draft.downloads,'id'))
    }
  ...
  componentDidMount() {
    this._downloadStoreUnlisten = FileDownloadStore.listen(this._onDownloadStoreChange);
    Actions.downloadStateChanged.listen(this._onDownloadStateChanged);

    if (this.props.session) {
      this._receivedNewSession();
    }

    this.props.draft.downloads.forEach((file) => {
      if(!_.contains(this.downloads, file.id)) {
  ...
  componentWillUnmount() {
    if(this._downloadStoreUnlisten)
      this._downloadStoreUnlisten();
  }

  componentWillReceiveProps(newProps) {
    console.log("ComposerView->componentWillReceiveProps");
  ...
  _onDownloadStoreChange = () => {
    console.log("ComposerView->_onDownloadStoreChange");
    console.log(this);

    this.setState({downloads: FileDownloadStore.downloadDataForFiles(_.pluck(this.props.draft.downloads,'id'))});
  }

  _onDownloadStateChanged = (download) => {
    console.log("ComposerView->_onDownloadStateChanged");

    console.log(download.state);
    if(download.state == Download.State.Finished) {
      console.log("Download Finished");
      console.log(download);
      DraftStore.sessionForClientId(this.props.draft.clientId).then ((session)=> {
        downloads = session.draft().downloads;
        console.log("Session Downloads");
        for(index in downloads) {
          d = downloads[index];
          console.log("Download Item")
          console.log(index);
          console.log(d);
          if(d.id == download.fileId) {
            console.log("Found download");
            downloads.splice(index, 1); break;
          }
        }
        ...
        Actions.addAttachment({filePath: download.targetPath, messageClientId:this.props.draft.clientId});
      });
    }
    // if(this.download && _.contains(this.download.ids, download.fileId)) {
    //   if(download.state == Download.State.Finished) {
    //     Actions.addAttachment({filePath: download.targetPath, messageClientId: this.props.draft.clientId});
    //     this.download.finishedCnt++;
    //   } else if (download.state == Download.State.Failed) {
    //     this.download.failedCnt++;
    //   }
    //
    //   if(this.download.finishedCnt+this.download.failedCnt == this.download.ids.length)
    //     this.state.downloading = false;
    // }

  }
  _renderContentScrollRegion() {
    if (PlanckEnv.isComposerWindow()) {
      return (
  ...
  _renderActionsRegion() {
    //console.log("_renderActionsRegion");
    //console.log(this.download);
    //console.log(this.state.downloading);
    //console.log(this.download&&this.state.downloading);
    return (
  ...
  _isDownloading = () => {
    return this.download && this.state.downloading
  }
